...
Based on these requirements, the power (\autoref{eq_bat_pow_req}) and energy (\autoref{eq_bat_energy_req}) requirements can be extracted by using an estimated overall efficiency of 0.8. 
\begin{align}
    P_{volt_{max}}
...
\ReqRef{R:Calibration} requires a means to calibrate the output values of the tester. Therefore a communication interface must be implemented to allow sending and receiving commands and measurement values. 
...
\todo[inline]{Check if the table is still required and remove it if not needed any longer. Otherwise consider updating it. }
%\STautoround*{2}
