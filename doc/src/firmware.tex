...
\paragraph{Charging (CC/CV).} CHARGE is auto-entered from IDLE when a
\qtyrange{15}{24}{\volt} supply is present, no \ac{BMS} fault is latched,
and the pack is below the end voltage. It runs constant-current until the
end voltage, then latches (one-way) into a constant-voltage tail, ending
once the pack is at end voltage \emph{and} current has tapered below
\qty{225}{\milli\ampere}. The most recent firmware pass (commit
\texttt{cb868d6}) replaced the CV-phase's ramped reference with a stepped
one and switched its feedback from stack voltage to the highest individual
cell voltage for more precise regulation, and added a live \ac{CLI}
\ac{ADC} readout during charging for bench visibility. \autoref{sec:fmea}
covers the still-open CC$\to$CV sensor mismatch and the untuned-gains risk.
...
\begin{table}[h!]
    ...
    \begin{tabular}{p{3cm}p{6cm}p{2.5cm}}
        ...
        \ac{LCD} (240$\times$320) & Display & \ac{SPI}4 (bit-banged) \\
    \end{tabular}
...
drives the \ac{LCD} by bit-banging, a \ac{PCB}-layout constraint. \ac{HRTIM}
